...
export class SharedService {
  ...
  constructor(
    ...
    this.store.select(selectResdata).subscribe((value: Restaurant) => {
      ...
      this.restaurantPromo = [];
      if(value.restaurant_data?.discounts && value.restaurant_data?.discounts?.length > 0) {
        value.restaurant_data?.discounts.map((discount: any) => {
          let data: Promo = {
            promoType: 'discount',
            _id: discount._id,
            title: discount.discount_title ? discount.discount_title : null,
            description: discount.discount_description ? discount.discount_description : null,
            terms_and_condition: discount.terms_and_condition ? discount.terms_and_condition : null,
            ...
            start_time: discount.start_time ? discount.start_time : null,
            end_time: discount.end_time ? discount.end_time : null,
            available_days: discount.discount_available_time ? this.getArrayOfObjectToFieldValue(discount.discount_available_time, 'day_name') : [],
            eligible_amount: discount.discount_eligible_amount ? Number(discount.discount_eligible_amount) : null,
            eligible_for: discount.discount_eligible_for ? Number(discount.discount_eligible_for) : null,
            eligible_payment_methods: discount.eligible_payment_methods !== '' ? Number(discount.eligible_payment_methods) : 0,
            type: Number(discount.discount_type),
            amount: Number(discount.discount_amount),
            rate: Number(discount.discount_amount),
            is_active: discount.is_active,
            is_default: discount.is_default,
            is_first_order: discount.is_first_order,
            platform: discount.platform !== '' ? Number(discount.platform) : 0
          }
          this.restaurantPromo.push(data);
        });
      }
      if(value.restaurant_data?.offers && value.restaurant_data?.offers?.length > 0) {
        value.restaurant_data?.offers.map((offer: any) => {
          let data: Promo = {
            promoType: 'offer',
            _id: offer._id,
            title: offer.offer_title ? offer.offer_title : null,
            description: offer.offer_description ? offer.offer_description : null,
            terms_and_condition: offer.terms_and_condition ? offer.terms_and_condition : null,
            image: offer.offer_image ? offer.offer_image : null,
            start_date: offer.start_date ? offer.start_date : null,
            end_date: offer.end_date ? offer.end_date : null,
            start_time: offer.start_time ? offer.start_time : null,
            end_time: offer.end_time ? offer.end_time : null,
            available_days: offer.offer_available_time ? this.getArrayOfObjectToFieldValue(offer.offer_available_time, 'day_name') : [],
            eligible_amount: offer.offer_eligible_amount ? Number(offer.offer_eligible_amount) : null,
            eligible_for: offer.offer_eligible_for ? Number(offer.offer_eligible_for) : null,
            eligible_payment_methods: offer.eligible_payment_methods !== '' ? Number(offer.eligible_payment_methods) : 0,
            type: 0,
            amount: 0,
            rate: 0,
            is_active: offer.is_active,
            is_default: offer.is_default,
            is_first_order: offer.is_first_order,
            platform: offer.platform !== '' ? Number(offer.platform) : 0
          }
          this.restaurantPromo.push(data);
        });
      }
